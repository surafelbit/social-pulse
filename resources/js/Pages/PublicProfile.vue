...
const mediaPosts = computed(() =>
    (props.posts ?? []).filter((p) => p.image_path),
);
...
const follow = (userId) => {
    router.post(
        ...
        {},
        {
            ...
            onSuccess: () => {
                window.showToast(
                    props.isFollowing ? "Unfollowed user!" : "Followed user!",
                );
            },
        },
    );
...
<template>
    ...
    >
        ...
        <header class="sticky top-0 z-50 sp-header">
            <div
                class="max-w-3xl mx-auto px-4 md:px-6 h-16 flex items-center justify-between gap-4"
            >
                <div class="flex items-center gap-3">
                    ...
                    <div class="flex items-center gap-2">
                        <span
                            class="w-2.5 h-2.5 rounded-full bg-[#32cd32] shadow-[0_0_10px_rgba(50,205,50,0.6)]"
                        ></span>
                        <Link
                            href="/dashboard"
                            class="font-headline font-bold text-lg tracking-tight"
                            style="color: var(--sp-text)"
                            >Social<span class="sp-gradient-text"
                                >Pulse</span
                            ></Link
                        >
                    </div>
                ...
                <div class="flex items-center gap-3">
                    ...
                    <Link
                        ...
                        class="relative flex items-center justify-center w-8 h-8 rounded-full transition-colors hover:bg-[#32cd32]/10"
                    ...
                    >
                        ...
                        >
                            {{
                                $page.props.auth.unreadNotifications > 9
                                    ? "9+"
        ...
        <main class="max-w-3xl mx-auto px-4 md:px-6 py-8">
            ...
            <div class="sp-glass-card overflow-hidden mb-6 animate-rise-in">
                ...
                >
                    <div
                        class="absolute -top-6 -left-6 w-24 h-24 rounded-full opacity-30 blur-2xl"
                        style="background: #32cd32"
                    ></div>
                    <div
                        class="absolute -bottom-4 right-10 w-20 h-20 rounded-full opacity-20 blur-2xl"
                        style="background: #4ce346"
                    ></div>
                    <div
                        class="absolute bottom-0 left-0 right-0 h-0.5 sp-accent-bar"
                    ></div>
                </div>
                <div class="px-6 pb-6">
                    <div class="flex items-end justify-between -mt-8 mb-4">
                        ...
                        >
                            <button
                                @click="startChat(profileUser.id)"
                                class="flex items-center gap-2 px-4 py-2 rounded-full text-sm font-bold transition-all duration-200 bg-blue-600 text-white hover:bg-blue-700"
                            >
                                <svg
                                    xmlns="http://www.w3.org/2000/svg"
                                    class="w-4 h-4"
                    ...
                    <h1
                        class="text-2xl font-bold font-headline tracking-tight mb-0.5"
                        style="color: var(--sp-text)"
                    >
                        {{ profileUser.name }}
                    </h1>
                        ...
                        <div v-else class="space-y-2">
                            ...
                            <div class="flex items-center gap-2">
                                <button
                                    ...
                                    class="sp-btn-primary text-xs py-1.5 px-4"
                                >
                                    {{ isSavingBio ? "Saving..." : "Save" }}
                                </button>
                                <button
                                    @click="isEditingBio = false"
                                    class="sp-btn-outline text-xs py-1 px-3 border-gray-400 text-gray-400 hover:bg-gray-400/10 hover:text-gray-300"
                                >
                                    Cancel
                                </button>
                                <span
                                    class="text-xs font-mono text-[var(--sp-text-3)] ml-auto"
                                    >{{ bioText.length }}/160</span
                                >
                            </div>
                        </div>
                    </div>
                    <!-- Stats -->
                    <div
                        class="flex items-center justify-around gap-2 mt-4 pt-4"
                    ...
                    >
                        ...
                        <button
                            ...
                            class="text-center flex-1 hover:opacity-80 transition-opacity focus:outline-none"
                        >
                            <div class="text-lg font-bold text-[#32cd32]">
                                {{ followers?.length ?? 0 }}
                            </div>
            ...
            <!-- TABS -->
            <div
                class="flex items-center gap-1 mb-6 p-1 rounded-xl"
                style="
            ...
            >
                <button
                    ...
                    class="flex-1 flex items-center justify-center gap-2 py-2 px-4 rounded-lg text-sm font-semibold transition-all duration-200"
                    :class="
                        activeTab === 'posts'
                    ...
                    :style="
                        activeTab === 'posts'
                            ? 'background: linear-gradient(135deg, #32cd32, #006e0a); color: white;'
                            : 'color: var(--sp-text-3);'
                    "
                >
                    <svg
                    ...
                    <span
                        ...
                        :style="
                            activeTab === 'media'
                                ...
                                : 'background: var(--sp-primary-glow); color: #32cd32;'
                        "
                        >{{ mediaPosts.length }}</span
                    >
                </button>
            </div>
            ...
            <div v-if="activeTab === 'posts'">
                <div
                    ...
                    class="text-center py-16 sp-glass-card"
                >
                ...
                <div v-else class="space-y-4">
                    <article
                        v-for="post in posts"
                        :key="post.id"
                        class="sp-glass-card p-5 group animate-rise-in"
                    >
                        ...
                        >
                            ...
                            />
                            <div
                                class="absolute inset-0 opacity-0 hover:opacity-100 transition-opacity duration-200 flex items-center justify-center rounded-2xl"
                        ...
                        <div
                            class="flex items-center gap-4 mt-4 pt-3"
                            style="border-top: 1px solid var(--sp-border)"
                        >
                            <span
                            ...
                            >
                                <svg
                                    xmlns="http://www.w3.org/2000/svg"
                                    class="w-3.5 h-3.5 fill-red-400"
                                    viewBox="0 0 24 24"
                                >
                                    <path
            ...
            <div v-if="activeTab === 'media'">
                ...
                >
                    ...
                    <p class="text-sm" style="color: var(--sp-text-3)">
                        {{ profileUser.name }} hasn't shared any photos yet.
                    </p>
                </div>
                <div v-else class="media-grid">
                    ...
                    >
                        ...
                        <div class="media-overlay">
                            <div
                                class="flex items-center gap-4 text-white text-sm font-semibold"
                            >
                            ...
                            <p
                                v-if="post.content"
                                class="text-white/80 text-xs px-3 text-center line-clamp-2"
                            >
                                {{ post.content }}
                            </p>
                        </div>
        ...
        <Transition name="lightbox">
            ...
            >
                <button
                    @click="closeLightbox"
                    class="absolute top-4 right-4 w-10 h-10 rounded-full flex items-center justify-center text-white transition-all hover:bg-white/10"
                ...
                <img
                    :src="lightboxSrc"
                    alt="Full image"
                    class="lightbox-img max-w-full max-h-[90vh] object-contain rounded-2xl shadow-2xl"
                />
            </div>
        </Transition>
        ...
        <Transition name="modal-fade">
            <div
                v-if="activeModal"
                class="fixed inset-0 z-[90] flex items-center justify-center p-4 bg-black/60 backdrop-blur-md"
                @click.self="closeModal"
            >
                <div
                    class="w-full max-w-md rounded-2xl overflow-hidden shadow-2xl sp-glass-card animate-scale-up"
                    style="
                ...
                >
                    ...
                    >
                        <h3
                            class="font-headline font-bold text-lg capitalize"
                            style="color: var(--sp-text)"
                        >
                            {{ activeModal }}
                        </h3>
                        <button
                            @click="closeModal"
                            class="w-8 h-8 rounded-full flex items-center justify-center transition-all hover:bg-red-500/10 hover:text-red-400"
                            style="
                                color: var(--sp-text-3);
                                border: 1px solid var(--sp-border);
                            "
                    ...
                    <div class="max-h-[350px] overflow-y-auto p-4 space-y-4">
                        <div
                            v-if="filteredModalUsers.length === 0"
                        ...
                        <div v-else class="space-y-3.5">
                            ...
                            >
                                ...
                                >
                                    <div
                                        class="w-9 h-9 rounded-full flex items-center justify-center text-xs font-bold text-white uppercase shadow-sm flex-shrink-0"
                                        style="
                                            background: linear-gradient(
                                                135deg,
                                                #32cd32 0%,
                                ...
                                <button
                                    ...
                                    :class="
                                        user.isFollowing
                                            ? 'border border-[#32cd32] text-[#32cd32] hover:bg-red-500/10 hover:border-red-400 hover:text-red-400'
                                            : 'sp-btn-primary'
                                    "
                                >
                                    {{
